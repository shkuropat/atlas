...
func OpenDataChunkFileWOptions(
	...
	fWOpts.Compressor, err = NewCompressor(readCompression, fWOpts.DataChunkFile, writeCompression, fWOpts.DataChunkFile)
	if err != nil {
...
func (f *DataChunkFileWithOptions) Read(p []byte) (n int, err error) {
	...
	if f.Compressor.ReadEnabled() {
		...
		if f.DataChunkFile.TransportMetadata.GetPresentationOptions().HasCompression() {
			log.Tracef("reading compressed data")
			return f.Compressor.Read(p)
		}

		log.Warnf("unknown compression method %v", f.DataChunkFile.TransportMetadata.GetPresentationOptions().GetCompression().GetType())
