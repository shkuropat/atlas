...
func (f *DataChunkFile) logDataChunk(dataChunk *DataChunk) {
	...
	case
		...
		dataChunk.GetLast(),                                // Log the last one to complete transfer logging
	...
	compression := f.TransportMetadata.GetPresentationOptions().GetCompression()

	// Fetch offset of this chunk within the stream
	offset := "not specified"
	if dataChunk.HasOffset() {
		offset = fmt.Sprintf("%d", dataChunk.GetOffset())
	}
